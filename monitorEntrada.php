 ...
 function leer($apiUrl, $serverIP, $serverPort, $apiToken, $accessLevelIds, $dbHost, $dbUser, $dbPass, $dbName, $doorId)
{
    ...
    if ($data && isset($data['code']) && $data['code'] === 0) {
        foreach ($data['data'] as $event) {
            ...
            if ($eventName === "Acceso denegado" && $readerName === "AndenesCalama-2-Salida") {

                echo "⚠️ ACCESO DENEGADO: El usuario con PIN {$userPin} no tiene permiso.\n";
                ...
                if (verificarPagoSalida($dbHost, $dbUser, $dbPass, $dbName, $patente)) {
                    ...
                    if (verificarTiempoEstadia($dbHost, $dbUser, $dbPass, $dbName, $patente)) {
            ...
            if ($eventName === "Usuario no registrado"&& $readerName==="AndenesCalama-1-Entrada") {
...
function registrarEntradaParking($dbHost, $dbUser, $dbPass, $dbName, $fechaEntrada, $horaEntrada, $patente)
{
    ...
    $stmt = $conn->prepare("INSERT INTO movParking (fechaent, horaent, patente, estado, tarifa, tipo) VALUES (?, ?, ?, 'ingresado', 'anden', 'Anden')");
 ...
 */
function verificarTiempoEstadia($dbHost, $dbUser, $dbPass, $dbName, $patente)
{
    $conn = new mysqli($dbHost, $dbUser, $dbPass, $dbName);
    if ($conn->connect_error) {
        die("Error de conexión a la base de datos: " . $conn->connect_error);
    ...
    if($diferencia->i <= 30){
        echo "Tiempo de gracia menor a 30 min:$diferencia->i ";
...
function actualizarEstadoSalida($dbHost, $dbUser, $dbPass, $dbName, $patente)
{
    ...
    // Query para actualizar el estado de la última entrada de la patente
    $stmt = $conn->prepare("UPDATE movParking SET estado = 'salida sin costo' WHERE patente = ? ORDER BY idmov DESC LIMIT 1");
    $stmt->bind_param("s", $patente);
    $stmt->execute();

    // Verificar si la actualización fue exitosa
    if ($stmt->affected_rows > 0) {
        echo json_encode(["status" => "success", "message" => "Estado actualizado a 'salida sin costo'", "patente" => $patente], JSON_PRETTY_PRINT);
    } else {
